...
![Nyx demo video screenshot](demo/assets/nyx-demo-field-view-v2.png)
...
Nyx는 RGB/depth, thermal, ultrasonic, IMU 관측을 로컬에서 `route / contact / egress / device state`로 압축합니다.  
경로 신뢰도가 낮으면 ORIENT / REALIGN 카드로 마지막 검증 체크포인트와 임무 의도를 회복합니다.  
미확인 접촉이나 열원이 있으면 MARK / STANDOFF / CONFIRM 카드로 사람, 위험, 안전을 확정하지 않게 막습니다.  
위험이 해소되지 않으면 HANDOFF 카드로 로봇, 전문팀, 다른 팀이 이어받을 판단 근거를 남깁니다.
